...
const char *mqtt_client_name = "Client";    // Client connections cant have the same connection name, use any random name
...
void connectMQTTbroker() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // MQTT client to process incoming messages and maintain connection to the server
    if (mclient.connected()) {
      mclient.loop();
    }
  }
